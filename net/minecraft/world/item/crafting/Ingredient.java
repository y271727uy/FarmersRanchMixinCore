...
public class Ingredient implements Predicate<ItemStack> {
   ...
   protected Ingredient(Stream<? extends Ingredient.Value> values) {
      this.values = values.toArray((p_43933_) -> {
   ...
   public static Ingredient of(Stream<ItemStack> stacks) {
      return fromValues(stacks.filter((p_43944_) -> {
   ...
   public static Ingredient fromNetwork(FriendlyByteBuf buffer) {
      ...
      if (size == -1) return net.minecraftforge.common.crafting.CraftingHelper.getIngredient(buffer.readResourceLocation(), buffer);
      return fromValues(Stream.generate(() -> new Ingredient.ItemValue(buffer.readItem())).limit(size));
   }
   ...
   public static Ingredient fromJson(@Nullable JsonElement json, boolean canBeEmpty) {
      if (json != null && !json.isJsonNull()) {
         Ingredient ret = net.minecraftforge.common.crafting.CraftingHelper.getIngredient(json, canBeEmpty);
