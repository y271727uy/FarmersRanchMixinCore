...
public class RecipeManager extends SimpleJsonResourceReloadListener {
   ...
   public <C extends Container, T extends Recipe<C>> Optional<T> getRecipeFor(RecipeType<T> recipeType, C inventory, Level level) {
      return this.byType(recipeType).values().stream().filter((p_220266_) -> {
         return p_220266_.matches(inventory, level);
      }).findFirst();
   }

   public <C extends Container, T extends Recipe<C>> Optional<Pair<ResourceLocation, T>> getRecipeFor(RecipeType<T> recipeType, C inventory, Level level, @Nullable ResourceLocation lastRecipe) {
      Map<ResourceLocation, T> map = this.byType(recipeType);
      if (lastRecipe != null) {
         T t = map.get(lastRecipe);
         if (t != null && t.matches(inventory, level)) {
            return Optional.of(Pair.of(lastRecipe, t));
         }
      }

      return map.entrySet().stream().filter((p_220245_) -> {
         return p_220245_.getValue().matches(inventory, level);
   ...
   public <C extends Container, T extends Recipe<C>> List<T> getRecipesFor(RecipeType<T> recipeType, C inventory, Level level) {
      return this.byType(recipeType).values().stream().filter((p_220241_) -> {
         return p_220241_.matches(inventory, level);
      }).sorted(Comparator.comparing((p_270043_) -> {
         return p_270043_.getResultItem(level.registryAccess()).getDescriptionId();
   ...
   public <C extends Container, T extends Recipe<C>> NonNullList<ItemStack> getRemainingItemsFor(RecipeType<T> recipeType, C inventory, Level level) {
      Optional<T> optional = this.getRecipeFor(recipeType, inventory, level);
 ...
 */
   ...
   public static Recipe<?> fromJson(ResourceLocation recipeId, JsonObject json) {
      return fromJson(recipeId, json, net.minecraftforge.common.crafting.conditions.ICondition.IContext.EMPTY);
   }
   ...
   public static Recipe<?> fromJson(ResourceLocation recipeId, JsonObject json, net.minecraftforge.common.crafting.conditions.ICondition.IContext context) {
      ...
      }).fromJson(recipeId, json, context);
   }
   ...
   public static <C extends Container, T extends Recipe<C>> RecipeManager.CachedCheck<C, T> createCheck(final RecipeType<T> recipeType) {
      return new RecipeManager.CachedCheck<C, T>() {
         ...
         public Optional<T> getRecipeFor(C p_220278_, Level p_220279_) {
            ...
            Optional<Pair<ResourceLocation, T>> optional = recipemanager.getRecipeFor(recipeType, p_220278_, p_220279_, this.lastRecipe);
            if (optional.isPresent()) {
